...
def run_experiment(args):
    ...
    if stage1_path.exists() and not args.force_train:
        ...
        best_stage1 = copy.deepcopy(model)
    else:
        ...
        for epoch in range(args.epochs):
            ...
            f1 = valid_metrics["All"]["F1"]
            print(f"[Stage 1] epoch={epoch + 1:03d} val_acc={acc:.4f} val_f1={f1:.4f}")
            if acc > best_acc or (acc == best_acc and f1 > best_f1):
                best_stage1 = copy.deepcopy(model)
                best_acc, best_f1, patience = acc, f1, 0
            else:
                patience += 1
            if patience >= args.convergence_patience:
                break
        model.load_state_dict(best_stage1.state_dict())
        torch.save(model.state_dict(), stage1_path)

    print("Evaluating Stage 1...")
    s1_fused, s1_e1, s1_e2, s1_e3 = evaluate(best_stage1, test_loader, graph, args, stage=1)
    ...
    if stage2_path.exists() and not args.force_train:
        ...
        model.load_state_dict(remap_checkpoint_keys(checkpoint))
        best_stage2 = copy.deepcopy(model)
    else:
        print("Training Stage 2 gate...")
        for name, param in model.named_parameters():
            param.requires_grad = "gate_layer" in name
        gate_optimizer = Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr * 0.1, weight_decay=1e-2)
        init_metrics, _, _, _ = evaluate(model, valid_loader, graph, args, stage=2)
        best_acc = init_metrics["All"]["ACC"]
        best_f1 = init_metrics["All"]["F1"]
        best_stage2 = copy.deepcopy(model)
        patience = 0
        for epoch in range(args.stage2_epochs):
            ...
            acc = valid_metrics["All"]["ACC"]
            f1 = valid_metrics["All"]["F1"]
            print(f"[Stage 2] epoch={epoch + 1:03d} val_acc={acc:.4f} val_f1={f1:.4f}")
            if (acc - best_acc) > 1e-4 or (abs(acc - best_acc) <= 1e-4 and (f1 - best_f1) > 1e-4):
                best_stage2 = copy.deepcopy(model)
                best_acc, best_f1, patience = acc, f1, 0
            else:
                patience += 1
            if patience >= 10:
                break
        model.load_state_dict(best_stage2.state_dict())
        torch.save(model.state_dict(), stage2_path)

    print("Evaluating Stage 2...")
    s2_fused, s2_e1, s2_e2, s2_e3 = evaluate(best_stage2, test_loader, graph, args, stage=2)
    result = {
        "seed": str(args.fold),
